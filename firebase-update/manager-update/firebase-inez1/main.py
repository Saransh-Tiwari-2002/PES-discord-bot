...
def name_update():
    ...
    for y in dupe:
        ...
        try:
            if(name[0] in 'g'):
                ref_g.child(name.replace(' ','-')).update({'URL':url})
            elif(name[0] in 'h'):
                ref_h.child(name.replace(' ','-')).update({'URL':url})
            elif(name[0] in 'i'):
                ref_i.child(name.replace(' ','-')).update({'URL':url})    
            elif(name[0] in 'j'):
                ref_j.child(name.replace(' ','-')).update({'URL':url})    
            elif(name[0] in 'k'):
                ref_k.child(name.replace(' ','-')).update({'URL':url})
        except: 
            print(y)    
...
ref_k.child('update-finished').update({'date':datetime.now(IST).strftime('%d-%m-%y')})
ref_k.child('update-finished').update({'time':datetime.now(IST).strftime('%H:%M:%S')})
