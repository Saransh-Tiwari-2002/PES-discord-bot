...
app_d = firebase_admin.initialize_app(cred_obj_d, {'databaseURL':'Enter URL here'},name= 'app_d')
ref_d=db.reference('/', app_d)

cred_obj_e = firebase_admin.credentials.Certificate('manager-05.json')
app_e = firebase_admin.initialize_app(cred_obj_e, {'databaseURL':'Enter URL here'},name= 'app_e')
ref_e=db.reference('/', app_e)

cred_obj_f = firebase_admin.credentials.Certificate('manager-06.json')
app_f = firebase_admin.initialize_app(cred_obj_f, {'databaseURL':'Enter URL here'},name= 'app2_f')
        ...
        for y in temp.get().values():
            if(x in y.get('URL')): 
                ...
                if(Attacking_Area_off!=None):
                    print(name, Attacking_Style_off)
...
def name_update():
    ...
    for y in dupe:
        ...
        try:
            if(name[0] in 'd'):
                ref_d.child(name.replace(' ','-')).update({'URL':url})
                print(len(ref_d.get().keys()))
            elif(name[0] in 'e'):
                ref_e.child(name.replace(' ','-')).update({'URL':url})
            elif(name[0] in 'f'):
                ref_f.child(name.replace(' ','-')).update({'URL':url})
        except: 
            print(y)    
...
ref_f.child('update-finished').update({'date':datetime.now(IST).strftime('%d-%m-%y')})
ref_f.child('update-finished').update({'time':datetime.now(IST).strftime('%H:%M:%S')})
